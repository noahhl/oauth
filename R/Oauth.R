...
LoadCredentials <- function(consumer_key) {
  params <- (load(file=paste("~/.oauthparams_", consumer_key, ".Rdata", sep="")))
}
...
MakeRequest <- function(params, resource, method, request=NULL, noisy=FALSE) {
  ...
  uri <- ifelse(is.null(request), resource, paste(resource, "?", names(request), "=", utils::URLencode(request), sep=""))
